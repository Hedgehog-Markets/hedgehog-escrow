...
import type { HhEscrow } from "@idl/hh_escrow";
...
export const globalState = (() => {
  const [address] = PublicKey.findProgramAddressSync([Buffer.from("global")], program.programId);

  const authority = Keypair.fromSecretKey(
    Uint8Array.from([
      190, 156, 93, 38, 182, 223, 17, 79, 244, 122, 157, 254, 116, 127, 149, 59, 98, 121, 64, 98,
      226, 153, 88, 126, 202, 242, 63, 187, 189, 104, 123, 206, 146, 157, 191, 135, 21, 111, 205,
      114, 182, 102, 151, 154, 114, 124, 226, 152, 232, 67, 164, 38, 69, 247, 197, 191, 206, 71, 9,
      16, 215, 110, 40, 101,
    ]),
  );
  ...
  return {
    ...
    async fetch() {
      const { feeWallet, owner, feeCutBps } = await program.account.globalState.fetch(address);
 ...
 */
export function getYesTokenAccountAddress(market: Address): [PublicKey, number] {
 ...
 */
export function getUserPositionAddress(user: Address, market: Address): PublicKey {
  const [userPosition] = PublicKey.findProgramAddressSync(
    [Buffer.from("user"), translateAddress(user).toBuffer(), translateAddress(market).toBuffer()],
    program.programId,
  );
  return userPosition;
}
