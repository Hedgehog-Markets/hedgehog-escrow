...
describe("claim", () => {
  ...
  it("fails if the user provides the yes token account", async () => {
    ...
    await expect(
      claim().accounts({ userTokenAccount: yesTokenAccount }).signers([user]).rpc(),
  ...
  it("fails if the user provides the no token account", async () => {
    ...
    await expect(
      claim().accounts({ userTokenAccount: noTokenAccount }).signers([user]).rpc(),
    ).rejects.toThrowProgramError(ErrorCode.UserAccountCannotBeMarketAccount);
  });
  ...
  it("fails if the market is not finalized", async () => {
    ...
    await expect(claim().signers([user]).rpc()).rejects.toThrowProgramError(ErrorCode.NotFinalized);
  });
});
