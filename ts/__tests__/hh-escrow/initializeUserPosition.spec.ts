...
import { __throw, createInitMintInstructions, intoU64BN, unixTimestamp } from "@/utils";
  ...
  it("fails if the seeds are incorrect", async () => {
    ...
    await expect(
      program.methods
        ...
        .signers([user])
        .rpc(),
    ).rejects.toThrow("Cross-program invocation with unauthorized signer or writable account");
  });
});
