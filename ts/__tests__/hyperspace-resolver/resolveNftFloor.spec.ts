...
describe("resolve nft floor resolver", () => {
  ...
  it("fails if market is incorrect", async () => {
    expect.assertions(1);
    ...
    await expect(
      resolveNftFloor()
        ...
        .rpc(),
    ).rejects.toThrowProgramError(ErrorCode.IncorrectMarket);
  });
  ...
  it("fails if expire timestamp has not passed", async () => {
    ...
    await expect(resolveNftFloor().signers([authority]).rpc()).rejects.toThrowProgramError(
      ErrorCode.TimestampNotPassed,
    );
  });
});
